...
public class MCBHandlerList<TEvent extends MCBEvent<TEvent>> {
     ...
     */
    private final EnumMap<Order, ArrayList<MCBListener<TEvent>>> handlerslots;
    /**
     ...
     */
    private static ArrayList<MCBHandlerList> alllists = new ArrayList<MCBHandlerList>();
    ...
    public MCBHandlerList() {
        handlerslots = new EnumMap<Order, ArrayList<MCBListener<TEvent>>>(Order.class);
        for (Order o : Order.values()) {
            handlerslots.put(o, new ArrayList<MCBListener<TEvent>>());
        }
        alllists.add(this);
    }
     ...
     */
    public void register(MCBListener<TEvent> listener, Order order) {
     ...
     */
    public void unregister(MCBListener<TEvent> listener) {
        for (Order o : Order.values()) {
     ...
     */
    public void unregister(MCBListener<TEvent> listener, Order order) {
        if (handlerslots.get(order).contains(listener)) {
            baked = false;
    ...
    void bake() {
        ...
        ArrayList<MCBListener[]> handlerslist = new ArrayList<MCBListener[]>();
        ArrayList<Integer> handleridslist = new ArrayList<Integer>();
        for (Entry<Order, ArrayList<MCBListener<TEvent>>> entry : handlerslots.entrySet()) {
            Order orderslot = entry.getKey();

            ArrayList<MCBListener<TEvent>> list = entry.getValue();

            int ord = orderslot.getIndex();
            handlerslist.add(list.toArray(new MCBListener[list.size()]));
            handleridslist.add(ord);
        }
        handlers = handlerslist.toArray(new MCBListener[handlerslist.size()][]);
